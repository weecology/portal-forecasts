...
echo "Creating new version of existing record $RECORD_ID..."
DEPOSITION_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer $ZENODOTOKEN" \
    "$ZENODO_URL/deposit/depositions/$RECORD_ID/actions/newversion")

HTTP_STATUS=$(echo "$DEPOSITION_RESPONSE" | tail -n1)
BODY=$(echo "$DEPOSITION_RESPONSE" | sed '$d')

if [ "$HTTP_STATUS" -lt 200 ] || [ "$HTTP_STATUS" -ge 300 ]; then
    echo "Error creating new version (HTTP $HTTP_STATUS): $BODY"
    exit 1
fi
...
echo "Created new version with deposition ID: $DEPOSITION_ID"
ACTION="update"
...
# Extract the new record ID from the publish response
NEW_RECORD_ID=$(echo "$PUBLISH_BODY" | jq -r '.id // empty')
[ -z "$NEW_RECORD_ID" ] && NEW_RECORD_ID=$RECORD_ID

echo "Published new version!"
echo "Deposition ID: $DEPOSITION_ID"
if [ "${ZENODOENV}" = "sandbox" ]; then
    echo "DOI: 10.5072/zenodo.$NEW_RECORD_ID"
    echo "URL: https://sandbox.zenodo.org/record/$NEW_RECORD_ID"
else
    echo "DOI: 10.5281/zenodo.$NEW_RECORD_ID"
    echo "URL: https://zenodo.org/record/$NEW_RECORD_ID"
fi
